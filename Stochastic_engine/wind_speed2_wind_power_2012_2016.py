...
def wind_sim(sim_years, PNW_cap, CAISO_cap):
    ...
    # BPA represents 76.6% of installed wind power capacity in PNW
    M = np.column_stack((BPA,BPA*(1/.766),CAISO))
        
    df_cap = pd.read_excel('Synthetic_wind_power/cap_by_month.xlsx',sheet_name = 'wind',header=0)
    
    for i in range(0,sim_years):
        for j in range(0,12):
            b_cap = df_cap.loc[i*12+j,'BPA']
            c_cap = df_cap.loc[i*12+j,'CAISO']
            
            M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,0] = b_cap*M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,0]
            M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,1] = b_cap*M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,1]
            M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,2] = c_cap*M[i*8760+j*30*24:i*8760+j*30*24 + 30*24,2]
            
            if j > 10:
                M[i*8760 + 8758:i*8760 + 8760,0] = b_cap*M[i*8760 + 8758:i*8760 + 8760,0]
                M[i*8760 + 8758:i*8760 + 8760,1] = b_cap*M[i*8760 + 8758:i*8760 + 8760,1]
                M[i*8760 + 8758:i*8760 + 8760,2] = c_cap*M[i*8760 + 8758:i*8760 + 8760,2]
